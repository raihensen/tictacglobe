...
class Preprocessor:
    def __init__(
        ...
        # ------------------------------------------------------------------------------------------------------------------
        # Fix data format
        self.df["continent"] = self.df["continent"].fillna("NA")  # North America fix
        self.df["iso"] = self.df["iso"].fillna("NA")  # Namibia fix
        ...
        self.categories = [
            ...
            NominalCategory(
                ...
                difficulty=2,
                col="name",
                extractor=lambda x: x[-1].upper(),
            ),
            NominalCategory(
                self.df,
                key="capital_starting_letter",
                name="Capital starting letter",
                difficulty=1.5,
                col="capital",
                extractor=lambda x: x[0].upper(),
            ),
            NominalCategory(
                self.df,
                key="capital_ending_letter",
                name="Capital ending letter",
                difficulty=3,
                col="capital",
                extractor=lambda x: x[-1].upper(),
            ),
            MultiNominalCategory(
                self.df,
                key="flag_colors",
                name="Flag color",
                difficulty=1.5,
            ...
            TopNCategory(
                self.df,
                key="top_20_population",
                name="Top 20 Population",
                difficulty=1,
                col="population",
                n=20,
            ),
            BottomNCategory(
                self.df,
                key="bottom_20_population",
                name="Bottom 20 Population",
                difficulty=2,
                col="population",
                n=20,
            ),
            ...
            SimpleBooleanCategory(
                self.df,
                key="capital_not_largest",
                name="Capital not largest city",
                difficulty=2,
                col="capital_not_largest",
            ),
        ]
        ...
        self.values = pd.concat(
            [
                ...
                pd.DataFrame(
                    {
                        cat.alt_key: cat.alt_values
                        for cat in self.categories.values()
                        if cat.alt_values is not None
                    }
                ),
            ],
            axis=1,
        ...
        for cat in self.categories.values():
            if isinstance(cat, NominalCategory):
                ...
                cat.alt_sets = (
                    self.values.explode(column=cat.alt_key)
                    .groupby(by=cat.alt_key)["iso"]
                    .agg(sorted)
                )
            elif isinstance(cat, MultiNominalCategory):
                cat.sets = (
                    self.values.explode(column=cat.key)
                    .groupby(by=cat.key)["iso"]
                    .agg(sorted)
                )
                cat.alt_sets = (
                    self.values.explode(column=cat.alt_key)
        ...
        while True:
            ...
            category_contents = {
                cat.key: cat.sets.sum() for cat in self.categories.values()
            }
            ...
            retain = {
                c
                for c in contents
                if len([key for key, cc in category_contents.items() if c in cc]) >= 2
            }
        ...
        self.setkeys = sum(
            [
                [(cat.key, value) for value in cat.sets.index]
                for cat in self.categories.values()
            ],
            [],
        )
        ...
        if self.min_cell_size is not None:
            self.cell_info = self.cell_info[
                (self.cell_info["size"] >= self.min_cell_size)
            ]
        if self.max_cell_size is not None:
            self.cell_info = self.cell_info[
                (self.cell_info["size"] <= self.max_cell_size)
            ]
        cell_keys = self.cell_info.apply(
            lambda row: (
                (row["row_cat"], row["row_val"]),
                (row["col_cat"], row["col_val"]),
            ),
            axis=1,
        )
        self.cells = {
            key: (contents, alt_contents)
            for key, contents, alt_contents in zip(
                cell_keys, self.cell_info["contents"], self.cell_info["alt_contents"]
            )
        }
        ...
        self.setkeys = list(
            sorted(
                set(
                    pd.concat(
                        [
                            cell_keys.apply(lambda c: c[0]),
                            cell_keys.apply(lambda c: c[1]),
                        ]
                    ).tolist()
                )
            )
        ...
        print(
            f"Retained {len(self.cells)} cells (of size {self.min_cell_size}-{self.max_cell_size})"
        )
        if len(self.setkeys) < len(setkeys_old):
            print(
                f"This lead to the removal of {len(setkeys_old) - len(self.setkeys)} key/value sets."
            )
    ...
    def init_cell_contents(self, key1, value1, key2, value2, alt=False):
        ...
        if isinstance(cat1, NominalCategory) and isinstance(cat2, NominalCategory):
            if cat1.col == cat2.col and cat1.extractor and cat2.extractor:
                col = cat1.col
                altcol = cat1.alt_col
                dfx = self.df[self.df["iso"].isin(alt_contents)][
                    ["iso", col, altcol]
                ].copy()
                dfx["values"] = dfx.apply(
                    lambda row: [row[col]] + list(row[altcol]), axis=1
                )
                dfx["src1"] = dfx["values"].apply(
                    lambda xx: [x for x in xx if cat1.extractor(x) == value1]
                )
                dfx["src2"] = dfx["values"].apply(
                    lambda xx: [x for x in xx if cat2.extractor(x) == value2]
                )
                dfx["keep"] = dfx.apply(
                    lambda row: not set(row["src1"]).isdisjoint(row["src2"]), axis=1
                )
    ...
    def get_generator(
        ...
        category_probs,
        seed=None,
        selection_mode="shuffle_categories",
        uniform=False,
        shuffle=True,
    ):
    ...
    def save_games(self, games, name: str):
        ...
        json.dump(
            {cat.key: cat.to_json() for cat in self.categories.values()},
        ...
        json.dump(
            [game.to_json() for game in games], open(path, mode="w", encoding="utf-8")
        )
