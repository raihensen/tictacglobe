...
def linear_combination(df, weights) -> pd.Series:
    return sum(w * df[col] for col, w in weights.items())  # type: ignore
...
class DifficultyEstimator:
    ...
    def compute_country_difficulties(self):
        ...
        df["difficulty"] = normalized_combination(
            pd.DataFrame(
                {
                    "sqrt_name_length": normalize(
                        np.sqrt(df["name"].apply(len)), scale=10
                    ),
                    "log_population": normalize(np.log(df["population"]), scale=10),
                    "log_gdp": normalize(np.log(df["gdp"]), scale=10),
                    "log_gdp_per_capita": normalize(
                        np.log(df["gdp_per_capita"]), scale=10
                    ),
                }
            ),
            COUNTRY_DIFFICULTY_WEIGHTS,
        ...
        df.loc[df["name"] == "Vatican", "difficulty"] = df.loc[
            df["name"] == "Monaco", "difficulty"
        ].iloc[0]
    ...
    def compute_category_difficulties(self):
        ...
        category_info = pd.Series(
            {
                (key, value): df[df[(key, value)]].difficulty.agg(list)
                for key, value in setkeys
            }
        )
        category_info = category_info.reset_index()
        category_info.columns = ["key", "value", "difficulties"]
        cat_difficulty = normalize(
            category_info.key.apply(lambda key: categories[key].difficulty), scale=10
        )
        category_info["difficulty"] = self.compute_solution_difficulties(
            category_info["difficulties"],
            CATEGORY_SOLUTION_DIFFICULTY_WEIGHTS,
            offsets=cat_difficulty,
        )
        category_info["countries"] = category_info.apply(
            lambda row: categories[row["key"]].sets.loc[row["value"]], axis=1
        )
    ...
    def compute_cell_difficulties(self, category_info):
        ...
        cell_info = cell_info.join(
            category_info["difficulty"].rename("row_difficulty"),
            on=["row_cat", "row_val"],
        )
        cell_info = cell_info.join(
            category_info["difficulty"].rename("col_difficulty"),
            on=["col_cat", "col_val"],
        )
        cell_info["row_col_difficulty"] = normalize(
            cell_info["row_difficulty"] + cell_info["col_difficulty"], scale=10
        )
        # cell_info["row_col_difficulty_harmonic"] = normalize((cell_info["row_difficulty"] + 1) * (cell_info["col_difficulty"] + 1), scale=10)

        content_difficulties = cell_info["contents"].apply(
            lambda cc: df[df.iso.isin(cc)].difficulty.agg(list)
        )
    ...
    def get_cell_indices(cell_data, cell_info):
        for (cat1, val1), (cat2, val2) in itertools.product(
            cell_data["rows"], cell_data["cols"]
        ):
            (row_cat, row_val), (col_cat, col_val) = max(
                (cat1, val1), (cat2, val2)
            ), min((cat1, val1), (cat2, val2))
            yield cell_info.index[
                (cell_info["row_cat"] == row_cat)
                & (cell_info["row_val"] == row_val)
                & (cell_info["col_cat"] == col_cat)
                & (cell_info["col_val"] == col_val)
            ].tolist()[0]
    ...
    def compute_game_difficulties(self, games):
            ...
            [
                {
                    ...
                    "cols": [(cat.key, value) for cat, value in game.cols],
                }
                for game in games
            ]
        )

        game_info["cell_indices"] = game_info.apply(
            lambda cell_data: list(self.get_cell_indices(cell_data, self.cell_info)),
            axis=1,
        )
        game_info["cell_difficulties"] = game_info["cell_indices"].apply(
            lambda ix: self.cell_info.loc[ix, "difficulty"].tolist()
        )
        ...
        game_info["num_unique"] = game_info["game"].apply(
            lambda game: len(
                [1 for solutions in sum(game.solutions, []) if len(solutions) == 1]
            )
        )

        # Final computation and Level assignment
        game_info["difficulty"] = normalized_combination(
            game_info, GAME_DIFFICULTY_WEIGHTS, scale=10
        )
