...
def add_flag_colors(df):
    # Assign colors
    colors = pd.read_csv("../../public/data/local/flag_colors/flag-colors.csv", sep=";")
    colors.columns = ["country", "color"]
    colors["country"] = colors["country"].ffill()
    colors.dropna(inplace=True)
    colors = colors.map(lambda x: x.strip())
    ...
    colors2 = pd.merge(
        df[["name"]], colors1, how="left", left_on="name", right_on="country"
    )
    colors2_outer = pd.merge(
        df[["name"]],
        colors1,
        how="outer",
        left_on="name",
                ...
                for mode, cc in match.groupdict().items():
                    if cc:
                        yield (
                            iso,
                            mode,
                            [parse_color(c.strip()) for c in cc.split(",")],
                        )
            else:
                print(spec, "no match")

    cfre = re.compile(
        r"^(?:(?:\[(?P<main_set>[^\(\)]*)\])|(?:(?P<main_add>[^\(\)]*)))?(?:,?\s*\((?P<optional>[^\(\)]*?)\))?,?\s*(?:\(\((?P<ignore>[^\(\)]*?)\)\))?$"
    )

    color_fixes = (
        open("../../public/data/local/flag_colors/flag color fixes.txt")
        .read()
        .split("\n")
    )
    ...
    if "flag_colors_alt" in list(df.columns):
        df["flag_colors_alt"] = df["flag_colors_alt"].apply(
            lambda cc: list(sorted(set(cc)))
        )
